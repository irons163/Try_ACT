...
public class Map {
	...
	final static class Coin {
		...
		public Coin(LightImage img, Map map) {
			this.map = map;
			this.coinImg = img;
			this.count = 0;
		}

		public void draw(long elapsedTime, MyCanvas g, int offsetX, int offsetY) {
			...
			this.lastCoinX = this.firstCoinX
					+ map.pixelsToTilesWidth(CommonUtil.screenWidth) + 1;
			...
			this.lastCoinY = this.firstCoinY
					+ map.pixelsToTilesHeight(CommonUtil.screenHeight) + 1;
	...
	final static class Item {
		...
		public Item(LightImage img, Map map) {
			this.map = map;
			this.load();
			this.itemImg = img;
		}
		...
		public void draw(MyCanvas g, int offsetX, int offsetY) {
			for (int i = 0; i < this.map.getICount(); ++i) {
				if (this.itemX[i] > 0 && this.map.getCount(i) > 0) {
					g.drawImage(this.itemImg, this.itemX[i] + offsetX,
							this.itemY[i] + offsetY);
				}
			}
		}
	...
	public Map(LightImage block, LightImage toge, LightImage ibox, LightImage springImg,
			LightImage goal, LightImage half, LightImage tile1, LightImage tile2,
			String filename) {
	...
	public void draw(MyCanvas g, int offsetX, int offsetY) {
		...
		int lastTileX = firstTileX
				+ pixelsToTilesWidth(CommonUtil.screenWidth) + 1;
		...
		int lastTileY = firstTileY
				+ pixelsToTilesHeight(CommonUtil.screenHeight) + 1;
		...
		for (i = firstTileY; i < lastTileY; ++i) {
			for (int j = firstTileX; j < lastTileX; ++j) {
				switch (this.map[i][j]) {
				case 45:
					g.drawImage(this.blockImg, tilesToWidthPixels(j)
							+ offsetX, tilesToHeightPixels(i) + offsetY);
					break;
				case 48:
					g.drawImage(this.tile2Img, tilesToWidthPixels(j)
							+ offsetX, tilesToHeightPixels(i) + offsetY);
					break;
				case 65:
					g.drawImage(this.togeImg,
				...
				case 68:
					...
					break;
				case 71:
					g.drawImage(this.goalImg,
							tilesToWidthPixels(j) + offsetX,
							tilesToHeightPixels(i) + offsetY);
					break;
				case 72:
					g.drawImage(this.halfImg,
							tilesToWidthPixels(j) + offsetX,
							tilesToHeightPixels(i) + offsetY);
					break;
				case 84:
					g.drawImage(this.tile1Img,
							tilesToWidthPixels(j) + offsetX,
							tilesToHeightPixels(i) + offsetY);
					break;
				case 88:
					g.drawImage(this.springImg, tilesToWidthPixels(j)
							+ offsetX, tilesToHeightPixels(i) + offsetY);
					break;
				case 89:
					g.drawImage(this.tile1Img,
		...
		for (i = 0; i < this.iCount; ++i) {
			g.drawImage(this.iboxImg, this.iboxX[i] + offsetX, this.iboxY[i]
					+ offsetY, tileWidth, tileHeight,
					this.count[i] * tileWidth, 0, this.count[i] * tileWidth
