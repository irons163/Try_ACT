...
public class Enemy {
	...
	private LightImage enemyImg;

	private LightImage enemyImg_F;

	private Map map;

	public Enemy(LightImage img, LightImage img2, Map map) {
		this.map = map;
		this.load();
		this.enemyImg = img;
		...
		for (int i = 0; i < this.map.getECount(); ++i) {
			if (this.enemyX[i] > 0) {
				if (this.reversef[i]) {
					switch (this.eType[i]) {
					case 0:
						g.drawImage(this.enemyImg, this.enemyX[i] + offsetX,
	...
	public void drawScore(MyCanvas g, int offsetX, int offsetY) {
		for (int i = 0; i < this.map.getECount(); ++i) {
			if (this.dethf[i]) {
//				g.setColor(GLColor.white);
//				g.setFont(LFont.getFont("Dialog", 0, 10));
				if (this.score[i] * this.combo[i] > 1000) {
					g.drawText("1000", this.enemyX[i] + offsetX + 10,
							this.enemyY[i] + offsetY, null);
				} else {
					g.drawText("" + this.score[i] * this.combo[i],
